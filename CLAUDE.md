...
- **Incoming peer messages:** When you see a message starting with `[peer ·` in the conversation (delivered via system-reminder), it's from another Claude session. The `reply to:` field contains their tab ID. To find your own tab ID: call `list_tabs`, match your project dir against Playwright MCP paths (local install = you). Respond directly using `send_to_tab <reply-to-id> "[peer · <project> — reply to: <your-tab-id>]: <reply>"` — no need for the user to relay. Answer briefly, then continue your current task.
