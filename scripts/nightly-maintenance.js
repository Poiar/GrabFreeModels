...
const STEP_NAMES = [
  ...
  'inherit-families',
  'fetch-huggingface-hub',
  'fetch-fastchat-registry',
  'fetch-hf-model-cards',
  'fetch-crfm-ecosystem',
...
(async () => {
    ...
    try {
      ...
      if (degraded.length === 0) {
        ...
        for (const d of degraded) {
          ...
          console.log(
            `    ${color}${d.full_id}\x1b[0m — ${d.status} on ${d.tested_at} (was working for ${d.streak} consecutive tests)`,
          );
        }
      }
    } catch (e) {
  ...
  await runStep('check-router-only-models', async () => {
    ...
    console.log(
      `  Router-only models: ${routerOnly.length} (stored in metadata._router_only_models)`,
    );
  ...
  await runStep('build-routing-graph', async () => {
    ...
    console.log(
      `  Routing graph: ${routerCount} routers mapped (stored in metadata._provider_routing_graph)`,
    );
    for (const [router, backends] of Object.entries(routing)) {
      console.log(
        `    ${router} → ${backends.length} backend(s), ${backends.reduce((s, b) => s + b.shared_models, 0)} shared models`,
      );
    }
    return payload;
  });
  ...
  await runStep('build-provider-timeline', async () => {
    ...
    console.log(
      `  Timeline: ${entries.length} dates, ${cumulative} total providers (stored in metadata._provider_timeline)`,
    );
    return payload;
  });
  ...
  await runStep(
    ...
    async () => {
      ...
      if (shouldRollback) {
        ...
        console.log(
          '  NOTE: Rollback only restores JSON export file, NOT the PostgreSQL database.',
        );
