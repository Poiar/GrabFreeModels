#!/usr/bin/env node
/**
 * Validates available-models.json structure and consistency.
 * Checks:
 *  1. Valid JSON
...
// 5. _role_rankings references valid model IDs
...
for (const [role, ids] of Object.entries(rankings)) {
  if (typeof ids !== 'object' || ids === null || Array.isArray(ids)) {
    ...
    // skip non-array values like "description"
  }
}
...
for (const key of summaryResultKeys) {
  ...
  for (const entry of ids) {
    // schema_issues entries are "modelId — detail" strings
