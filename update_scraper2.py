...
function extractModels(html) {
  ...
  for (let i = 1; i < rowParts.length; i++) {
    ...
    models.push({
      ...
      intelligence,
      price: parseFloat((cells[5] || '').replace('$', '').replace(',', '')) || null,
      speed: parseFloat(cells[6]) || null,
      ttft: parseFloat(cells[7]) || null,
      total: parseFloat(cells[8]) || null,
    });
  }
  return models;
}
...
with open('scripts/scrape-artificial-analysis.js', 'w', encoding='utf-8', newline='') as f:
    f.write(content)
print('Written scraper:', len(content), 'bytes')
