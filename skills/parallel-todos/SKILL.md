...
2. **Pick the right agent per item**:
   - `subagent_type: "explore"` — read-only tasks: research, search, analysis, summarization. Can be unreliable with free models; falls back to `general` on failure.
   - `subagent_type: "general"` — write tasks: edit files, implement features, run commands. Also works for read-only tasks.
...
- Subagent fallback models only work in TUI sessions, NOT when orchestrating via Task tool. Don't rely on fallbacks to save a hung subagent.
- If a subagent returns empty or clearly failed, retry once with the same prompt (transient errors are common).
