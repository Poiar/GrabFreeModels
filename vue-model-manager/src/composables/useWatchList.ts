...
export function useWatchList() {
  ...
  function isWatched(superId: number): boolean {
    return watchedIds.value.has(superId);
  }

  function add(model: ModelData) {
    if (isWatched(model.super_id)) return;
    const entry: WatchedModel = {
  ...
  const changes = computed(() => {
    ...
    for (const item of items.value) {
      ...
      if (!model) continue;
      const curr = model.providers.some((p) => !p._removed && p.status.result === 'working')
        ? 'working'
        : model.providers.some((p) => !p._removed && p.status.result === 'rate_limited')
          ? 'rate_limited'
          : 'broken';
