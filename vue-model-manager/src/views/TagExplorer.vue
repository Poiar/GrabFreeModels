<template>
  <div class="te-page">
    <div class="page-header">
      <h2>Tag Explorer</h2>
      <p>
        Discover models by capability tags — click any tag to see matching models and related tags
      </p>
    </div>
    ...
    <div class="te-stats">
      <div class="te-stat">
        <span class="te-stat-val">{{ tagCount }}</span
        ><span class="te-stat-lbl">Unique tags</span>
      </div>
      <div class="te-stat">
        <span class="te-stat-val">{{ taggedModelCount }}</span
        ><span class="te-stat-lbl">Tagged models</span>
      </div>
      <div class="te-stat">
        <span class="te-stat-val">{{ avgTagsPerModel }}</span
        ><span class="te-stat-lbl">Avg tags/model</span>
      </div>
    </div>
    ...
    <div class="te-cloud">
      <span
        ...
        :class="{
          ...
          disabled: selectedTags.length > 0 && !selectedTags.includes(tag.name) && !tag.related,
        }"
        :style="{
          fontSize: 0.65 + (tag.count / maxCount) * 1.2 + 'rem',
          opacity:
            selectedTags.length === 0
              ? 1
              : selectedTags.includes(tag.name) || tag.related
                ? 1
                : 0.3,
        }"
    ...
    <div v-if="selectedTags.length === 1 && coOccurring.length" class="te-co-section">
      ...
      <div class="te-co-chips">
        <span
          v-for="[tag, count] in coOccurring"
          :key="tag"
          class="te-co-chip"
          @click="addTag(tag)"
        >
      ...
      <div class="te-model-grid">
        ...
        >
          ...
          <div class="te-model-tags">
            <span
              v-for="tag in m.best_for"
              :key="tag"
              class="te-model-tag"
              :class="{ highlight: selectedTags.includes(tag) }"
              >{{ tag }}</span
            >
          </div>
        </router-link>
