  ...
  <div v-if="provider" class="pd-page">
    <div class="page-header">
      ...
      <div class="md-header-row">
        ...
        <div class="md-header-actions">
          <router-link
            v-if="orgForProvider"
            :to="`/org/${orgForProvider.id}`"
            class="md-org-link"
            title="View unified organization page"
            >Org view →</router-link
          >
          <button
            class="md-copy-btn"
            @click="copyProviderAsMarkdown(provider, totalModels)"
            title="Copy as Markdown"
          >
            ↓ MD
          </button>
          <button class="md-copy-btn" @click="copyAsJson(provider)" title="Copy as JSON">
            ↓ JSON
          </button>
      ...
      <div class="pd-facts" v-if="providerFacts.length">
        <span v-for="f in providerFacts" :key="f.label" class="pd-fact-chip" :class="f.cls">{{
          f.label
        }}</span>
      </div>
      <p v-if="provider.description" class="pd-description">{{ provider.description }}</p>

      <div v-if="isPaidProvider" class="pd-presumed-banner">
        Paid provider — all models presumed working (not tested).
      </div>
    ...
    <div class="pd-meta-grid">
      ...
      <div class="cd-stat" v-if="provider.provider_type">
        <span class="cd-stat-value cd-stat-type" :class="provider.provider_type">{{
      ...
      <div
        class="cd-stat"
        v-if="provider.provider_type === 'inference' && provider.serves_third_party !== null"
      >
        <span
          class="cd-stat-value cd-stat-host"
          :class="provider.serves_third_party ? 'host' : 'firstparty'"
          >{{ provider.serves_third_party ? 'Open-model host' : 'First-party only' }}</span
        >
        <span class="cd-stat-label">Model scope</span>
      </div>
      <div class="cd-stat" v-if="provider.hardware && provider.hardware !== 'unknown'">
        <span class="cd-stat-value cd-stat-hw" :class="provider.hardware">{{
      ...
      <div class="cd-stat" v-if="provider.max_daily_requests">
        <span class="cd-stat-value cd-stat-rate"
    ...
    <div class="cd-validation-bar">
      <div
        ...
        :style="{ flex: hbFlex.working }"
        :title="counts.working + ' working'"
      ></div>
      <div
        class="val-segment rate_limited"
        :style="{ flex: hbFlex.rate_limited }"
        :title="counts.rate_limited + ' rate limited'"
      ></div>
      <div
        class="val-segment broken"
        :style="{ flex: hbFlex.broken }"
        :title="counts.broken + ' broken'"
      ></div>
      <div
        class="val-segment untested"
        :style="{ flex: hbFlex.untested }"
        :title="counts.untested + ' untested'"
      ></div>
    </div>
    <div class="cd-val-legend">
      <span v-if="counts.working" class="val-legend working"
        >{{ counts.working }} working ({{ pctWorking }}%)</span
      >
      <span v-if="counts.rate_limited" class="val-legend rate_limited"
        >{{ counts.rate_limited }} rate limited</span
      >
    ...
    <div v-if="quantEntries.length > 0" class="pd-quant-section">
      ...
      <div class="pd-quant-chips">
        <span
          v-for="[format, count] in quantEntries"
          :key="format"
          class="pd-quant-chip"
          :class="quantClass(format)"
        >
    ...
    <div v-if="failureEntries.length > 0" class="pd-fail-section">
      ...
      <div class="pd-fail-chips">
        <span
          v-for="[cat, count] in failureEntries"
          :key="cat"
          class="pd-fail-chip"
          :class="'fail-' + cat"
        >
    ...
    <div v-if="providerCreators.length === 0" class="pd-empty">
      No models found for this provider.
    </div>
    <div v-for="{ creator, models } in providerCreators" :key="creator.id" class="pd-creator-group">
      <h4 class="pd-creator-name">
        <router-link :to="`/creator/${creator.id}`" class="pd-creator-link">{{
          creator.name
        }}</router-link>
        <span class="pd-creator-count"
...
const provider = computed(() =>
  store.providerRefs.find((p: { slug: string }) => p.slug === providerSlug.value),
);
const orgForProvider = computed(() => store.getOrgByProviderSlug(providerSlug.value));
...
const rateLimitText = computed(() => {
  ...
  if (p.max_tpm)
    parts.push(
      p.max_tpm >= 1000000
        ? `${(p.max_tpm / 1000000).toFixed(1)}M TPM`
        : `${p.max_tpm?.toLocaleString()} TPM`,
    );
  return parts.join(' / ');
});
  ...
  for (const { models } of providerCreators.value) {
    for (const m of models) {
      for (const dp of m.providers) {
        ...
        if (dp.is_free) return false;
      }
    }
  }
  return totalModels.value > 0;
});
...
const providerFacts = computed(() => {
  ...
  // Parameter range
  const paramSizes = [...new Set(dps.map((dp) => dp.param_count_b).filter(Boolean))].sort(
    (a, b) => a - b,
  );
  ...
  if (families.size > 0) {
    chips.push({
      label: `${families.size} ${families.size === 1 ? 'family' : 'families'}`,
      cls: 'fact-family',
    });
  }
  ...
  for (const { models } of providerCreators.value) {
    for (const m of models) {
      ...
      if (
        ...
        workingProviders[0].provider_slug === providerSlug.value
      ) {
        exclusive++;
      }
    }
  ...
  if (exclusive > 0) {
    chips.push({
      label: `${exclusive} exclusive model${exclusive !== 1 ? 's' : ''}`,
      cls: 'fact-exclusive',
    });
  }
  ...
  const hw = dps.find(
    (dp) => dp.hardware && dp.hardware !== 'gpu' && dp.hardware !== 'unknown',
  )?.hardware;
  if (hw) {
    const hwLabel =
      hw === 'lpu'
  ...
  const releaseDates = dps
    .map((dp: any) => dp.release_date)
    .filter(Boolean)
    .sort();
...
const counts = computed(() => {
  ...
  return {
    ...
    rate_limited,
    untested,
    total: working + broken + rate_limited + untested,
  };
});
...
.pd-fact-chip.fact-param {
  background: rgba(99, 102, 241, 0.12);
  color: #818cf8;
}
.pd-fact-chip.fact-family {
  background: rgba(52, 211, 153, 0.12);
  color: #34d399;
}
.pd-fact-chip.fact-creator {
  background: rgba(236, 72, 153, 0.12);
  color: #ec4899;
}
.pd-fact-chip.fact-exclusive {
  background: rgba(245, 158, 11, 0.12);
  color: #f59e0b;
}
.pd-fact-chip.fact-hardware {
  background: rgba(168, 85, 247, 0.12);
  color: #a855f7;
}
.pd-fact-chip.fact-compat {
  background: rgba(59, 130, 246, 0.12);
  color: #60a5fa;
}
.pd-fact-chip.fact-streaming {
  background: rgba(52, 211, 153, 0.12);
  color: #34d399;
}
.pd-fact-chip.fact-account {
  background: rgba(245, 158, 11, 0.12);
  color: #f59e0b;
}
.pd-fact-chip.fact-card {
  background: rgba(239, 68, 68, 0.12);
  color: #f87171;
}
.pd-fact-chip.fact-fresh {
  background: rgba(168, 85, 247, 0.12);
  color: #a855f7;
}
...
.pd-fail-summary-chip.fail-sum-timeout {
  background: rgba(251, 191, 36, 0.12);
  color: #fbbf24;
}
.pd-fail-summary-chip.fail-sum-not_found {
  background: rgba(156, 163, 175, 0.12);
  color: #9ca3af;
}
.pd-fail-summary-chip.fail-sum-auth_error {
  background: rgba(239, 68, 68, 0.15);
  color: #f87171;
}
.pd-fail-summary-chip.fail-sum-rate_limited {
  background: rgba(245, 158, 11, 0.12);
  color: #f59e0b;
}
.pd-fail-summary-chip.fail-sum-server_error {
  background: rgba(239, 68, 68, 0.12);
  color: #ef4444;
}
.pd-fail-summary-chip.fail-sum-network_error {
  background: rgba(59, 130, 246, 0.12);
  color: #60a5fa;
}
.pd-fail-summary-chip.fail-sum-unknown {
  background: rgba(156, 163, 175, 0.12);
  color: #9ca3af;
}
...
.cd-stat-type.inference {
  color: #60a5fa;
}
.cd-stat-type.local {
  color: #34d399;
}
.cd-stat-type.discovery {
  color: #fbbf24;
}
.cd-stat-host {
  font-size: 0.72rem;
...
.pd-quant-chip strong {
  font-family: 'JetBrains Mono', monospace;
}
.pd-quant-chip.qt-full {
  background: rgba(52, 211, 153, 0.12);
  color: #34d399;
}
.pd-quant-chip.qt-mid {
  background: rgba(251, 191, 36, 0.12);
  color: #fbbf24;
}
.pd-quant-chip.qt-low {
  background: rgba(239, 68, 68, 0.12);
  color: #f87171;
}
.pd-quant-chip.qt-unk {
  background: rgba(156, 163, 175, 0.12);
  color: #9ca3af;
}
.pd-quant-chip.qt-other {
  background: rgba(59, 130, 246, 0.12);
  color: #60a5fa;
}
...
.pd-fail-chip.fail-timeout {
  background: rgba(251, 191, 36, 0.12);
  color: #fbbf24;
}
.pd-fail-chip.fail-not_found {
  background: rgba(156, 163, 175, 0.12);
  color: #9ca3af;
}
.pd-fail-chip.fail-auth_error {
  background: rgba(239, 68, 68, 0.15);
  color: #f87171;
}
.pd-fail-chip.fail-rate_limited {
  background: rgba(245, 158, 11, 0.12);
  color: #f59e0b;
}
.pd-fail-chip.fail-server_error {
  background: rgba(239, 68, 68, 0.12);
  color: #ef4444;
}
.pd-fail-chip.fail-network_error {
  background: rgba(59, 130, 246, 0.12);
  color: #60a5fa;
}
.pd-fail-chip.fail-unknown {
  background: rgba(156, 163, 175, 0.12);
  color: #9ca3af;
}
...
.pd-tier-chip.tier-top {
  background: rgba(245, 158, 11, 0.12);
  color: #f59e0b;
}
.pd-tier-chip.tier-high {
  background: rgba(59, 130, 246, 0.12);
  color: #60a5fa;
}
.pd-tier-chip.tier-mid {
  background: rgba(99, 102, 241, 0.12);
  color: #818cf8;
}
.pd-tier-chip.tier-basic {
  background: rgba(156, 163, 175, 0.12);
  color: #9ca3af;
}
.pd-tier-chip.tier-unknown {
