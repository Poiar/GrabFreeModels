...
<style scoped>
.creator-detail-page { max-width: 1200px; margin: 0 auto; padding: 20px; }
...
.cd-aggregate {
  display: flex; gap: 24px; padding: 12px 16px; margin: 16px 0;
  border: 1px solid var(--border); border-radius: 8px; background: var(--bg-card);
}
.cd-stat { display: flex; flex-direction: column; }
.cd-stat-value { font-size: 1.1rem; font-weight: 700; color: var(--accent); }
.cd-stat-label { font-size: 0.68rem; color: var(--text-muted); }

.section-title { font-size: 1rem; font-weight: 700; margin: 20px 0 12px; }
.cd-models { display: flex; flex-direction: column; gap: 8px; }

.cd-not-found { padding: 40px 20px; text-align: center; color: var(--text-muted); }
