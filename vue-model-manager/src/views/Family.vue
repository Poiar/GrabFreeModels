  ...
  <div>
    ...
    <div class="super-list-controls">
      ...
      <div class="super-list-count">
        {{ sortedItems.length }} of {{ familyItems.length }} families
      </div>
    </div>
...
const familyItems = computed<FamilyItem[]>(() => {
  const map = new Map<string, { masters: any[], allProviders: Set<string>, working: number, untested: number, rateLimited: number, broken: number }>()
  ...
  for (const m of store.superModels) {
    ...
    for (const fam of families) {
      if (!map.has(fam)) map.set(fam, { masters: [], allProviders: new Set(), working: 0, untested: 0, rateLimited: 0, broken: 0 })
  ...
  for (const [family, entry] of map) {
    items.push({
      family,
      modelCount: entry.masters.length,
      instanceCount: entry.masters.reduce((s, m) => s + m.datapoints.length, 0),
      providers: [...entry.allProviders].sort(),
      memberNames: entry.masters.map(m => m.name).sort(),
