<template>
  <div class="cp-page">
    ...
    <div class="cp-selectors">
      ...
      <button class="cp-swap-btn" @click="swap" title="Swap providers" :disabled="!slugA || !slugB">⇄</button>
      <div class="cp-select-group">
    ...
    <div v-else class="cp-grid">
      ...
      <div class="cp-cell">
        <span v-if="provA" class="cp-tag" :class="provA.provider_type || 'unknown'">{{ typeLabel(provA) }}</span>
      </div>
      <div class="cp-cell">
        <span v-if="provB" class="cp-tag" :class="provB.provider_type || 'unknown'">{{ typeLabel(provB) }}</span>
      </div>

      <!-- Hardware -->
      <div class="cp-row-header">Hardware</div>
      <div class="cp-cell">
        <span v-if="provA" class="cp-tag" :class="provA.hardware || 'unknown'">{{ hwLabel(provA) }}</span>
      </div>
      <div class="cp-cell">
        <span v-if="provB" class="cp-tag" :class="provB.hardware || 'unknown'">{{ hwLabel(provB) }}</span>
      </div>

      <!-- Country -->
      <div class="cp-row-header">Country</div>
      <div class="cp-cell">
        <span v-if="provA" class="cp-country" :style="{ color: getCountryForProvider(provA.slug).text, background: getCountryForProvider(provA.slug).color }">{{ getCountryForProvider(provA.slug).name }}</span>
      </div>
      <div class="cp-cell">
        <span v-if="provB" class="cp-country" :style="{ color: getCountryForProvider(provB.slug).text, background: getCountryForProvider(provB.slug).color }">{{ getCountryForProvider(provB.slug).name }}</span>
      </div>

      <!-- Health -->
      <div class="cp-row-header">Health</div>
      <div class="cp-cell">
        <span v-if="provA" class="cp-status" :class="provA.health_status">{{ provA.health_status }}</span>
      </div>
      <div class="cp-cell">
        <span v-if="provB" class="cp-status" :class="provB.health_status">{{ provB.health_status }}</span>
      </div>

      <!-- Model scope -->
      <div class="cp-row-header">Model scope</div>
      <div class="cp-cell">
        <span v-if="provA" class="cp-tag" :class="provA.serves_third_party === false ? 'firstparty' : 'host'">{{ scopeLabel(provA) }}</span>
      </div>
      <div class="cp-cell">
        <span v-if="provB" class="cp-tag" :class="provB.serves_third_party === false ? 'firstparty' : 'host'">{{ scopeLabel(provB) }}</span>
      </div>

      <!-- OpenAI compat -->
      <div class="cp-row-header">API compat</div>
      <div class="cp-cell">
        <span v-if="provA">{{ provA.is_openai_compat !== false ? 'OpenAI-compatible' : 'Non-standard' }}</span>
      </div>
      <div class="cp-cell">
        <span v-if="provB">{{ provB.is_openai_compat !== false ? 'OpenAI-compatible' : 'Non-standard' }}</span>
      </div>
      ...
      <div class="cp-cell cp-cell-num">
        ...
        <div v-if="provA" class="cp-bar-track">
          <div class="cp-bar-fill" :style="{ width: provA.model_count ? (provA.working_count / provA.model_count * 100) + '%' : '0%' }" :class="provA.health_status"></div>
        </div>
      </div>
      <div class="cp-cell cp-cell-num">
        <strong v-if="provB">{{ provB.model_count }}</strong>
        <div v-if="provB" class="cp-bar-track">
          <div class="cp-bar-fill" :style="{ width: provB.model_count ? (provB.working_count / provB.model_count * 100) + '%' : '0%' }" :class="provB.health_status"></div>
        </div>
      </div>
      ...
      <div class="cp-cell cp-cell-num">
        ...
        <span v-if="provA" class="cp-pct">({{ provA.model_count ? Math.round(provA.working_count / provA.model_count * 100) : 0 }}%)</span>
      </div>
      <div class="cp-cell cp-cell-num">
        <span v-if="provB" class="cp-val working">{{ provB.working_count }}</span>
        <span v-if="provB" class="cp-pct">({{ provB.model_count ? Math.round(provB.working_count / provB.model_count * 100) : 0 }}%)</span>
      </div>
      ...
      <div class="cp-cell">
        <template v-if="provA && provB">
          ...
          <div v-if="sharedModels.length" class="cp-model-list">
            <span v-for="m in sharedModels.slice(0, 8)" :key="m.super_id" class="cp-model-chip shared">{{ m.name }}</span>
            <span v-if="sharedModels.length > 8" class="cp-model-chip more">+{{ sharedModels.length - 8 }} more</span>
          </div>
        </template>
      </div>
      <div class="cp-cell">
      ...
      <div class="cp-cell">
        <template v-if="provA && provB">
          ...
          <div v-if="uniqueToA.length" class="cp-model-list">
            <span v-for="m in uniqueToA.slice(0, 8)" :key="m.super_id" class="cp-model-chip unique">{{ m.name }}</span>
            <span v-if="uniqueToA.length > 8" class="cp-model-chip more">+{{ uniqueToA.length - 8 }} more</span>
          </div>
        </template>
      </div>
      <div class="cp-cell">
        <template v-if="provA && provB">
          ...
          <div v-if="uniqueToB.length" class="cp-model-list">
            <span v-for="m in uniqueToB.slice(0, 8)" :key="m.super_id" class="cp-model-chip unique">{{ m.name }}</span>
            <span v-if="uniqueToB.length > 8" class="cp-model-chip more">+{{ uniqueToB.length - 8 }} more</span>
          </div>
        </template>
      </div>
...
const providers = computed((): ProviderReference[] =>
  [...store.visibleProviderRefs].sort((a, b) =>
    b.model_count - a.model_count || a.name.localeCompare(b.name),
  ),
);
...
function hwLabel(p: ProviderReference): string {
  const labels: Record<string, string> = {
    gpu: 'GPU', lpu: 'LPU', wafer: 'Wafer-scale', tpu: 'TPU', edge: 'Edge', local: 'Local', unknown: '—',
  };
...
<style scoped>
.cp-page { max-width: 1000px; margin: 0 auto; padding: 20px; }
...
.cp-swap-btn:hover:not(:disabled) { color: var(--text); border-color: var(--text-dim); }
.cp-swap-btn:disabled { opacity: 0.3; cursor: default; }
...
.cp-col-header.cp-col-active { color: var(--accent); }
.cp-cell {
  padding: 8px 14px;
  background: var(--bg-card);
  font-size: 0.78rem;
...
.cp-cell-num strong { font-size: 1.1rem; font-family: 'JetBrains Mono', monospace; }
.cp-sep {
  grid-column: 1 / -1;
  height: 8px;
  background: var(--bg);
...
.cp-tag.router { background: rgba(139,92,246,0.12); color: #A78BFA; }
.cp-tag.inference { background: rgba(59,130,246,0.12); color: #60A5FA; }
.cp-tag.local { background: rgba(16,185,129,0.12); color: #34D399; }
.cp-tag.discovery { background: rgba(245,158,11,0.12); color: #FBBF24; }
.cp-tag.unknown { background: rgba(156,163,175,0.12); color: #9CA3AF; }
.cp-tag.gpu { background: rgba(245,158,11,0.12); color: #F59E0B; }
.cp-tag.lpu { background: rgba(139,92,246,0.12); color: #A78BFA; }
.cp-tag.wafer { background: rgba(244,114,182,0.12); color: #F472B6; }
.cp-tag.tpu { background: rgba(59,130,246,0.12); color: #60A5FA; }
.cp-tag.edge { background: rgba(16,185,129,0.12); color: #34D399; }
.cp-tag.firstparty { background: rgba(245,158,11,0.12); color: #F59E0B; }
.cp-tag.host { background: rgba(16,185,129,0.12); color: #34D399; }
...
.cp-status.healthy { background: rgba(63,185,80,0.12); color: var(--green); }
.cp-status.degraded { background: rgba(251,191,36,0.12); color: var(--orange); }
.cp-status.down { background: rgba(248,113,113,0.12); color: var(--red); }

/* Values */
.cp-val { font-family: 'JetBrains Mono', monospace; }
.cp-val.working { color: var(--green); }
.cp-val.free { color: var(--accent); }
.cp-pct { font-size: 0.65rem; color: var(--text-dim); margin-left: 4px; }
...
.cp-bar-fill { height: 100%; border-radius: 2px; transition: width 0.4s; }
.cp-bar-fill.healthy { background: var(--green); }
.cp-bar-fill.degraded { background: var(--orange); }
.cp-bar-fill.down { background: var(--red); }
...
@media (max-width: 768px) {
  ...
  .cp-selectors { flex-direction: column; }
  .cp-swap-btn { align-self: center; transform: rotate(90deg); }
}
</style>
