<template>
  <div class="af-page">
    <div class="page-header">
      <h2>Activity Feed</h2>
      <p>Recent model status changes across all providers</p>
    </div>
    ...
    <div v-else class="af-timeline">
      <div v-for="(evt, i) in filteredEvents" :key="i" class="af-event">
        ...
        <span class="af-event-type" :class="'af-type-' + evt.type">{{
          evt.type === 'broken'
            ? '↓ went broken'
            : evt.type === 'fixed'
              ? '↑ came back'
...
interface FeedEvent {
  ...
  name: string;
  provider: string;
  type: 'broken' | 'fixed' | 'rate_limited';
}

const events = computed((): FeedEvent[] => {
  const result: FeedEvent[] = [];
  // Recently broken
  for (const r of store.recentlyBroken)
    result.push({
      date: 'latest',
      slug: r.slug,
      name: r.name,
  ...
  // Recently fixed
  for (const r of store.recentlyFixed)
    result.push({
      date: 'latest',
      slug: r.slug,
      name: r.name,
  ...
  result.sort((a, b) => {
    const o: Record<string, number> = { broken: 0, fixed: 1, rate_limited: 2 };
