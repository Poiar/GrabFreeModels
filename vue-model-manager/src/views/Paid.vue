...
const paidModels = computed(() => store.paidModels)
...
function readQueryFromUrl() {
  if (route.query.q && typeof route.query.q === 'string') jql.rawQuery.value = route.query.q
}
...
function syncBuilderToQuery(c: BuilderCondition[]) {
  if (!c.length) { jql.rawQuery.value = ''; return }
  let q = c[0].jql
  for (let i = 1; i < c.length; i++) { q += c[i].joinOr ? ' OR ' : ' AND '; q += c[i].jql }
...
function onLoadSavedQuery(e: Event) {
  jql.rawQuery.value = (e as CustomEvent).detail as string
...
function exportCsv() {
  ...
  download([
    ...
    `# count: ${sortedItems.value.length}/${paidModels.value.length}`,
    `# query: ${jql.rawQuery.value || '(none)'}`,
    `# sort: ${sortBy.value} ${sortDesc.value ? 'DESC' : 'ASC'}`,
    'id,name,author,provider,status,context_length,input_price_per_million,output_price_per_million,type,best_for,tools,notes,status_detail,status_tested,last_success,removed,removed_date',
    ...rows,
...
function exportJson() {
  const data = sortedItems.value.map(m => ({
    ...
    context_length: m.context_length, input_price_per_million: m.input_price_per_million ?? null,
  ...
  download(JSON.stringify({
    _meta: {
      ...
      total_models: paidModels.value.length, jql_query: jql.rawQuery.value || null,
      sort_by: sortBy.value, sort_desc: sortDesc.value,
    },
    models: data,
  }, null, 2), 'paid-models.json', 'application/json')
}
