  ...
  <div>
    ...
    <div class="super-list-controls">
      <div class="super-list-search">
        ...
        <input v-model="searchQuery" type="text" placeholder="Search by author or model name…" spellcheck="false" />
    ...
    <div class="table-wrap vscroll-table super-list-table">
      <div class="vscroll-header-row">
        <div class="vscroll-header-cell col-author-name sortable" :class="{ sortActive: sortBy === 'name' }" @click="setSort('name')">
          Author <SortArrow :active="sortBy === 'name'" :desc="sortDesc" />
        </div>
...
<p>No authors match your current search or filters.</p>
...
const searchedItems = computed(() => {
  ...
  if (!q) return authorItems.value
  return authorItems.value.filter(a =>
    a.author.toLowerCase().includes(q) ||
    a.memberNames.some(n => n.toLowerCase().includes(q))
  )
})
...
const sortedItems = computed(() => {
  ...
  arr.sort((a, b) => {
    let cmp = 0
    switch (sortBy.value) {
      case 'name': cmp = a.author.localeCompare(b.author); break
...
const workingCount = computed(() => authorItems.value.filter(i => i.workingCount > 0 && i.brokenCount === 0 && i.rateLimitedCount === 0).length)
const partialCount = computed(() => authorItems.value.filter(i => i.workingCount > 0 && (i.brokenCount > 0 || i.rateLimitedCount > 0)).length)
const untestedCount = computed(() => authorItems.value.filter(i => i.untestedCount > 0 && i.workingCount === 0).length)
const brokenCount = computed(() => authorItems.value.filter(i => i.workingCount === 0 && i.untestedCount === 0).length)
...
.col-author-name { width: 16%; min-width: 120px; }
...
.col-members      { width: 35%; min-width: 200px; }
