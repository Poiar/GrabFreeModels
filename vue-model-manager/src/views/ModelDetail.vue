<template>
  <div v-if="model" class="md-page">
    ...
    <div class="page-header">
      ...
      <p class="md-subtitle">
        <router-link :to="`/creator/${creatorSlug}`" class="md-link">{{
          model.creator || 'Unknown'
        }}</router-link>
        <template v-if="model.base_model">
          · derived from
          <router-link :to="`/supermodel/${model.base_model}`" class="md-link">{{
            model.base_model
          }}</router-link>
          <span v-if="model.derivation_method" class="md-deriv-badge">{{
            formatDerivMethod(model.derivation_method)
          }}</span>
        </template>
        <template v-if="model.family">
          ·
          <router-link :to="`/family/${encodeURIComponent(model.family)}`" class="md-link">{{
            formatFamilyName(model.family)
          }}</router-link>
        </template>
        <template v-if="model.base_model">
          ·
          <router-link :to="`/base-model/${model.base_model}`" class="md-link"
            >base model</router-link
          >
        </template>
      </p>
      <!-- Facts -->
      <div class="md-facts" v-if="modelFacts.length">
        <span v-for="f in modelFacts" :key="f.label" class="md-fact-chip" :class="f.cls">{{
          f.label
        }}</span>
      </div>
    ...
    <div class="md-meta-grid">
      ...
      <div class="md-stat">
        <span class="md-stat-value">{{ isFreeContext ? validationPct + '%' : '—' }}</span>
        <span class="md-stat-label">{{ isFreeContext ? 'Validation' : 'Status' }}</span>
      </div>
    ...
    <div v-if="!isFreeContext" class="md-paid-notice">
      ...
      <div class="md-rankings-grid">
        ...
        >
          ...
          <div v-if="roleScores[role]" class="md-rank-score-details">
            <span v-if="roleScores[role].qualityBonus" class="md-score-chip"
              >Quality: {{ roleScores[role].qualityBonus?.toFixed(1) }}</span
            >
            <span v-if="roleScores[role].freshness !== undefined" class="md-score-chip"
              >Fresh: {{ (roleScores[role].freshness * 100).toFixed(0) }}%</span
            >
            <span v-if="roleScores[role].score !== undefined" class="md-score-chip score"
              >Score: {{ roleScores[role].score?.toFixed(1) }}</span
            >
          </div>
        </div>
      </div>
    ...
    <div class="md-provider-table">
      ...
      >
        ...
        <div class="md-dp-details">
          ...
          <span v-if="dp.knowledge_cutoff" class="md-dp-detail cutoff">{{
            dp.knowledge_cutoff
          }}</span>
          <span v-if="dp.hardware && dp.hardware !== 'unknown'" class="md-dp-detail hw">{{
            dp.hardware
          }}</span>
        </div>
        <div class="md-dp-limits" v-if="dp.max_rpm || dp.max_tpm || dp.max_daily_requests">
          ...
          <span v-if="dp.max_daily_requests" class="md-dp-limit"
    ...
    <div v-if="modelIssues.length" class="md-issues">
      ...
      <div v-for="issue in modelIssues" :key="issue.model_id" class="md-issue-card">
        ...
        <p v-if="issue.workaround" class="md-issue-workaround">
...
const roleScores = computed(() => {
  if (!model.value)
    return {} as Record<string, { score?: number; qualityBonus?: number; freshness?: number }>;
...
const modelIssues = computed(() => {
  if (!model.value) return [];
  return store.knownIssues.filter(
    (i: {
      model_id: string;
      issue: string;
      impact: string;
...
const modelFacts = computed(() => {
  ...
  if (dp?.param_count_b)
    chips.push({ label: formatParams(dp.param_count_b) + ' params', cls: 'fact-param' });
  if (model.value.best_context)
    chips.push({ label: formatContext(model.value.best_context) + ' ctx', cls: 'fact-ctx' });
  ...
  if (openCount === activeDatapoints.value.length && openCount > 0)
    chips.push({ label: 'Open weights', cls: 'fact-open' });
  else if (openCount > 0)
    chips.push({
      label: `${openCount}/${activeDatapoints.value.length} open`,
      cls: 'fact-partial',
    });
  if (dp?.knowledge_cutoff)
    chips.push({ label: 'Knowledge: ' + dp.knowledge_cutoff, cls: 'fact-cutoff' });
...
.md-fact-chip.fact-param {
  background: rgba(99, 102, 241, 0.12);
  color: #818cf8;
}
.md-fact-chip.fact-ctx {
  background: rgba(52, 211, 153, 0.12);
  color: #34d399;
}
.md-fact-chip.fact-deriv {
  background: rgba(236, 72, 153, 0.12);
  color: #ec4899;
}
.md-fact-chip.fact-open {
  background: rgba(52, 211, 153, 0.12);
  color: #34d399;
}
.md-fact-chip.fact-partial {
  background: rgba(251, 191, 36, 0.12);
  color: #eab308;
}
.md-fact-chip.fact-cutoff {
  background: rgba(168, 85, 247, 0.12);
  color: #a855f7;
}
.md-fact-chip.fact-thinking {
...
.md-dp-status.working {
  color: var(--green);
  background: color-mix(in srgb, var(--green) 10%, transparent);
}
.md-dp-status.rate_limited {
  color: var(--orange);
...
.md-dp-status.not_found {
  color: var(--text-dim);
  background: var(--bg-elevated);
}
.md-dp-status.untested {
  color: var(--accent);
  background: var(--accent-subtle);
}
.md-dp-details {
...
.md-issue-severity.critical {
  color: #ef4444;
  background: rgba(239, 68, 68, 0.12);
}
.md-issue-severity.high {
  color: #f97316;
  background: rgba(249, 115, 22, 0.12);
}
.md-issue-severity.moderate {
  color: #eab308;
  background: rgba(234, 179, 8, 0.12);
}
.md-issue-severity.low {
  color: var(--text-dim);
  background: var(--bg-elevated);
}
.md-issue-impact {
