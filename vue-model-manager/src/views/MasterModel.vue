<template>
  <div>
    ...
    <div v-else-if="notFound" class="center-message error-box">
      ...
      <p class="error-message">No master model with ID <code>{{ route.params.id }}</code>.</p>
      <router-link to="/all" class="refresh-btn">Browse all models</router-link>
    </div>

    <template v-else-if="master">
      <!-- Summary cards -->
      <div class="stats-grid master-stats">
        <div class="stat-card">
          <div class="stat-value">{{ master.datapoints.length }}</div>
        ...
        <div class="stat-card">
          <div class="stat-value">{{ master.any_tools ? 'Yes' : 'No' }}</div>
          <div class="stat-label">Tool Support</div>
        </div>
        <div class="stat-card">
          <div class="stat-value" :class="master.all_free ? 'green' : 'orange'">{{ master.all_free ? 'All Free' : 'Some Paid' }}</div>
      ...
      <!-- Known issues for any datapoint of this master -->
      <div class="section" v-if="masterIssues.length > 0">
        <h3>Known Issues</h3>
        <div v-for="issue in masterIssues" :key="issue.model_id + issue.issue" class="issue-card">
...
const master = computed(() => {
  if (isNaN(masterId.value)) return null
  return store.masterModelById.get(masterId.value) ?? null
})

const notFound = computed(() => store.allDatapoints.length > 0 && !master.value && !isNaN(masterId.value))

const masterFamily = computed(() => {
  if (!master.value) return null
  const families = new Set(master.value.datapoints.map(d => d.family).filter((f): f is string => !!f))
  return families.size === 1 ? Array.from(families)[0] : null
})

const sortedDatapoints = computed(() => {
  if (!master.value) return []
  return [...master.value.datapoints].sort((a, b) => {
    const statusOrder: Record<string, number> = { working: 0, untested: 1, rate_limited: 2, not_found: 3, broken: 4 }
    const aOrder = a._removed ? 5 : (statusOrder[a.status.result] ?? 9)
    const bOrder = b._removed ? 5 : (statusOrder[b.status.result] ?? 9)
    if (aOrder !== bOrder) return aOrder - bOrder
    return a.provider.localeCompare(b.provider)
  })
})

const workingCount = computed(() => master.value?.datapoints.filter(d => d.status.result === 'working').length ?? 0)
const rateLimitedCount = computed(() => master.value?.datapoints.filter(d => d.status.result === 'rate_limited').length ?? 0)
const brokenCount = computed(() => master.value?.datapoints.filter(d => d.status.result === 'broken').length ?? 0)
const untestedCount = computed(() => master.value?.datapoints.filter(d => d.status.result === 'untested').length ?? 0)
const bestContext = computed(() => master.value?.best_context_length ?? null)
...
const modelRoles = computed((): ModelRole[] => {
  ...
  for (const role of ROLE_ORDER) {
    ...
    for (const dp of master.value.datapoints) {
...
const masterIssues = computed(() => {
  if (!master.value) return []
  const ids = new Set(master.value.datapoints.map(d => d.id))
  return store.knownIssues.filter(i => ids.has(i.model_id))
})
