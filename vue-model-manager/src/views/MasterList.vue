<template>
  <div>
    <div class="page-header">
      <h2>Models</h2>
      <p>Browse all master models — click any model to see its full details across all providers</p>
    </div>
    ...
    <div class="master-list-controls">
      ...
      <div class="master-list-count">
        {{ filteredItems.length }} of {{ masterItems.length }} master models
      </div>
    </div>
    ...
    <!-- Table -->
    <div class="table-wrap vscroll-table master-list-table">
      ...
      >
        <template #default="{ item }">
          <div class="vscroll-row row-clickable" :class="{ 'row-has-removed': item.hasRemoved }" @click="$router.push(`/master/${item.id}`)" role="button" :title="'View ' + item.name">
            <div class="vscroll-cell col-name">
              <span class="model-name" :title="item.name">{{ item.name }}</span>
              <span v-if="item.family" class="master-family">{{ item.family }}</span>
            </div>
      ...
      <div v-else class="empty-state">
        <div class="empty-state-inner">
          <p>No master models match your search.</p>
        </div>
      </div>
    </div>
  </div>
...
const masterItems = computed<MasterItem[]>(() => {
  return store.masterModels.map(m => {
...
.master-list-search input::placeholder {
  color: var(--text-muted);
}
.search-clear {
  background: none;
