  ...
  <div class="sm-card" :class="`card-${status}`" @click="handleClick" role="button" tabindex="0">
    ...
    <div class="sm-name-row">
      <span class="sm-model-name">
        ...
        <span
          v-if="isDegraded"
          class="sm-degraded-warning"
          title="Stability degraded — was stable, now broken"
          >!</span
        >
        ...
        <router-link
          v-if="model.base_model || model.derivation_method"
          :to="model.base_model ? `/model/${model.base_model}` : ''"
          class="sm-finetune-badge"
    ...
    <div class="sm-tags-row">
      ...
      <span v-for="tier in uniqueTiers" :key="'tier-' + tier" class="sm-tier-chip">{{
        formatTier(tier)
      }}</span>
      ...
      <span v-if="efficiencyBadge" class="sm-eff-chip" :title="efficiencyBadge.title">{{
        efficiencyBadge.label
      }}</span>
      <span v-if="anyThinking" class="sm-thinking-chip">Thinking</span>
      <span v-if="uniqueStage" class="sm-stage-chip" :class="'stage-' + uniqueStage">{{
        formatStage(uniqueStage)
      }}</span>
    ...
    <div class="sm-stats">
      <span class="sm-stat"
        >{{ datapointsCount }} provider{{ datapointsCount !== 1 ? 's' : '' }}</span
      >
    ...
    <div
      v-if="healthSpark"
      class="sm-health-spark"
      :title="healthSpark.stability + '% stable · ' + healthSpark.streak + 'd streak'"
    >
      <svg viewBox="0 0 60 14" class="sm-spark-svg">
        <polyline
          ...
          fill="none"
          :stroke="
            healthSpark.stability >= 80
              ? 'var(--green)'
              : healthSpark.stability >= 50
                ? 'var(--orange)'
                : 'var(--red)'
          "
          stroke-width="1.5"
    ...
    <div class="sm-footer">
      <div class="sm-providers">
        <span
          v-for="p in providerTags.slice(0, 6)"
          :key="p.slug"
          class="provider-tag"
          :style="{
            background: getProviderColorMuted(p.slug),
            color: getProviderColorForeground(p.slug),
          }"
        >
          <ProviderIcon :slug="p.slug" :size="14" :cls="'sm-provider-logo'" />
          {{ p.name }}
        </span>
        <span v-if="providerTags.length > 6" class="provider-tag more"
          >+{{ providerTags.length - 6 }}</span
        >
      </div>
...
const efficiencyBadge = computed(() => {
  ...
  return {
    label: `Eff ${eff}`,
    title: `Cost efficiency: ${eff} (intelligence ÷ provider count). Higher = better quality per provider.`,
  };
});
...
const isDegraded = computed(() => activeDps.value.some((d) => store.degradedModels.has(d.full_id)));
  ...
  return [...idCounts.entries()]
    .map(([id, count]) => {
      ...
      return {
        key: s.slug,
        label,
        title: `${s.name} (${count} provider${count > 1 ? 's' : ''})`,
...
.sm-card.card-working {
  border-left-color: var(--green);
}
.sm-card.card-mixed {
  border-left-color: var(--orange);
}
.sm-card.card-deprecated {
  opacity: 0.55;
}
.sm-card.card-deprecated .sm-deprecated-name {
...
.sm-finetune-badge.deriv-ft {
  background: rgba(99, 102, 241, 0.12);
  color: var(--deriv-ft);
}
.sm-finetune-badge.deriv-merge {
  background: rgba(168, 85, 247, 0.12);
  color: var(--deriv-merge);
}
.sm-finetune-badge.deriv-distill {
  background: rgba(236, 72, 153, 0.12);
  color: var(--deriv-distill);
}
.sm-finetune-badge.deriv-dpo {
  background: rgba(34, 211, 238, 0.12);
  color: var(--deriv-dpo);
}
.sm-finetune-badge.deriv-cpt {
  background: rgba(250, 204, 21, 0.12);
  color: var(--deriv-cpt);
}
.sm-finetune-badge.deriv-lora {
  background: rgba(52, 211, 153, 0.12);
  color: var(--deriv-lora);
}
.sm-finetune-badge.deriv-unknown {
