<template>
  <div ref="rootRef" class="ss" :class="{ open: isOpen }">
    <div
      class="ss-trigger"
      role="button"
      tabindex="0"
      :class="{ 'ss-has-val': hasValue }"
      @click="toggle"
      @keydown.enter.prevent="toggle"
      @keydown.space.prevent="toggle"
    >
      <template v-if="multiple && (modelValue as string[]).length > 0">
        <span v-for="v in (modelValue as string[]).slice(0, 3)" :key="v" class="ss-multi-pill">{{
          optLabel(v)
        }}</span>
        <span v-if="(modelValue as string[]).length > 3" class="ss-multi-count"
          >+{{ (modelValue as string[]).length - 3 }}</span
        >
      </template>
    ...
    <div v-if="isOpen" class="ss-dropdown">
      <div v-if="options.length > 4" class="ss-search-wrap">
        <input
          ref="searchRef"
          v-model="search"
          type="text"
      ...
      <div class="ss-list">
        <div
          ...
          @click.stop="pick(opt)"
        >
          <span v-if="isSelected(opt.value)" class="ss-check">✓</span>
...
const hasValue = computed(() =>
  multiple.value ? (modelValue.value as string[]).length > 0 : !!modelValue.value,
);
...
function isSelected(v: string): boolean {
  return multiple.value ? (modelValue.value as string[]).includes(v) : modelValue.value === v;
}
