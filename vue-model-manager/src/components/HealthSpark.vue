<template>
  <div v-if="data" class="hs-wrap" :title="data.stability + '% stable · ' + data.latency_ms + 'ms avg'">
    <svg viewBox="0 0 40 12" class="hs-svg">
      <polyline
        :points="data.points.map(p => p.x + ',' + p.y).join(' ')"
        fill="none"
        :stroke="data.stability >= 80 ? 'var(--green)' : data.stability >= 50 ? 'var(--orange)' : 'var(--red)'"
        stroke-width="1.2"
...
const data = computed(() => {
  ...
  const avgLatency = latencies.length ? Math.round(latencies.reduce((a, b) => a + b.latency_ms!, 0) / latencies.length) : null;
...
.hs-svg { width: 40px; height: 12px; flex-shrink: 0; }
.hs-latency { font-size: 0.58rem; color: var(--text-muted); font-family: 'JetBrains Mono', monospace; white-space: nowrap; }
</style>
