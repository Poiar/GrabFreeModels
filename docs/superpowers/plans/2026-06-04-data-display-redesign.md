...
const providers = Array.from(providerRefMap.values())
  .map((ref) => ({
    ...
    health_status:
      ref.working_count === ref.model_count
        ? 'healthy'
        : ref.working_count > 0
          ? 'degraded'
          : 'down',
...
<template>
  <div class="provider-table-wrap">
    <table class="provider-table">
      ...
      <tbody>
        <tr
          v-for="dp in sortedProviders"
          :key="dp.full_id"
          class="pt-row"
          :class="`pt-${dp.status.result}`"
        >
...
<template>
  <div class="model-card" :class="{ 'card-expanded': expanded }" @click="handleCardClick">
    ...
    <div class="mc-stats">
      ...
      <span class="mc-stat-divider">|</span>
      <span class="mc-stat"
        >{{ model.providers.length }} provider{{ model.providers.length !== 1 ? 's' : '' }}</span
      >
  ...
  <div class="creator-list-page">
    ...
    <div class="creator-grid">
      ...
      >
        ...
        <div class="cc-top" v-if="creator.models.length">Top: {{ creator.models[0].name }}</div>
      </router-link>
    </div>
  </div>
</template>
